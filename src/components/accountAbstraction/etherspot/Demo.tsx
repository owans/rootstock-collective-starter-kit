...
export default function Demo() {
    ...
    // Demo-only: ephemeral EOA key generated in-browser, kept in memory, never sent to any server.
    ...
    const estimateAndTransfer = async () => {
        ...
        if (!isAddress(recipient.trim())) {
            toast({
                ...
                description: "Enter a valid Ethereum address for the recipient.",
                variant: "destructive",
            });
            return;
        }
        ...
        try {
            ...
            toast({
                title: "Invalid value",
                description: "Enter a valid positive number for the amount (e.g. 0.01).",
                variant: "destructive",
            });
            return;
        }
        ...
        try {
            ...
            await sdk.addUserOpsToBatch({ to: recipient.trim() as `0x${string}`, value: valueWei });
